...
#define RM_AR_RELEASE_MINOR_VERSION_C      4
#define RM_AR_RELEASE_REVISION_VERSION_C   0
#define RM_SW_MAJOR_VERSION_C              4
#define RM_SW_MINOR_VERSION_C              0
#define RM_SW_PATCH_VERSION_C              2
...
static Std_ReturnType Rm_ValidateSemaphoreChannelCall(Rm_Sema42_ChannelType ChannelNumber,
                                                                      uint8 u8ServiceId
                                                     );
...
void Rm_Init(Rm_ConfigType const * ConfigPtr)
{
    ...
    if ((Std_ReturnType)E_OK == valid)
    {
        ...
        if ((Std_ReturnType)E_OK == valid)
        {
            ...
            Rm_Ipw_Mpu_M7_MulticoreInit();
            (void)Rm_pConfig;
            #else
            Rm_Ipw_Mpu_M7_Init(Rm_pConfig->Rm_pIpwConfig);
            #endif
            #endif
...
*
* @param[in]    ChannelNumber   Semaphore channel
* @return       boolean* @retval       TRUE        if the reset gate is idle
 ...
 * @pre
 * */
/* @implements         Rm_Mpu_M7_MulticoreInit_Activity */
void Rm_Mpu_M7_MulticoreInit(void)
{
    Rm_Ipw_Mpu_M7_MulticoreInit();
}
#endif /* #if (RM_MPU_M7_MULTICORE_SUPPORT == STD_ON) */
 ...
 *
 * */
/* @implements         Rm_Mpu_M7_EnableRegion_Activity */
void Rm_Mpu_M7_EnableRegion(uint8 u8RegionNum, boolean bEnable)
{
    Rm_Ipw_Mpu_M7_EnableRegion(u8RegionNum, bEnable);
}
 ...
 *
 * */
/* @implements         Rm_Mpu_M7_SetAccessRight_Activity */
void Rm_Mpu_M7_SetAccessRight(uint8 u8RegionNum, Rm_Mpu_M7_AccessRightsType eRights)
{
    Rm_Ipw_Mpu_M7_SetAccessRight(u8RegionNum, eRights);
}
 ...
 *
 * */
/* @implements         Rm_Mpu_M7_GetErrorDetails_Activity */
Std_ReturnType Rm_Mpu_M7_GetErrorDetails(Rm_Mpu_M7_ErrorDetailsType * pErrorDetails)
{
    Std_ReturnType retval = (Std_ReturnType)E_NOT_OK;
    boolean bErr;
    bErr = Rm_Ipw_Mpu_M7_GetErrorDetails(pErrorDetails);
...
#endif /* #if (RM_ENABLE_MPU_M7 == STD_ON) */
